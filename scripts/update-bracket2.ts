...
function MatchCard({ node, openMatchDetails }: { node: BracketNode, openMatchDetails: (e: React.MouseEvent, id: string) => void }) {
  ...
  const getTeamRow = (teamId: string | null, score: number, isWinner: boolean, opacityClass: string) => {
    ...
    // Original style: winner gets bg-surface-variant border-l-2 border-primary, others get opacity
    return (
...
export default function BracketRoomClient({ contest, initialSnapshot }: { contest: ContestListingItem, initialSnapshot: BracketSnapshot }) {
  ...
  useEffect(() => {
    const drawLines = () => {
      ...
      // Use offset calculations relative to the absolute container
      ...
      for (let r = 0; r < initialSnapshot.totalRounds - 1; r++) {
        ...
        currentRoundNodes.forEach((node, i) => {
          ...
          if (el1 && el2) {
            ...
            // Calculate relative to the container element
    ...
    <>
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative dark stitch-container bg-background w-full h-full text-on-surface font-body-md selection:bg-primary-container selection:text-on-primary-container">
        ...
        <main className="flex-1 flex flex-col h-full overflow-hidden relative w-full">
          ...
          >
            {/* SVG Bracket Structure - Fixed bounds and relative absolute container */}
            <div id="bracket-container" className="min-w-[1400px] min-h-[900px] relative">
...
console.log("Updated BracketRoomClient.tsx with absolute bounds and correct styling!");

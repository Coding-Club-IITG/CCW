...
function MatchCard({ node, openMatchDetails }: { node: BracketNode, openMatchDetails: (e: React.MouseEvent, id: string) => void }) {
  ...
  let containerStyle = "bg-surface-container border border-outline-variant rounded-lg overflow-hidden hover:border-primary transition-colors cursor-pointer";
  if (isActive) {
    containerStyle = "bg-surface-container border border-primary rounded-lg overflow-hidden shadow-[0_0_15px_rgba(46,125,50,0.2)] cursor-pointer";
  } else if (isPending) {
    containerStyle = "bg-surface-container border border-outline-variant rounded-lg overflow-hidden opacity-60 hover:opacity-100 transition-opacity cursor-pointer";
  }
    ...
    <>
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative dark stitch-container bg-background w-full text-on-surface font-body-md selection:bg-primary-container selection:text-on-primary-container" style={{ height: 'calc(100vh - 64px)' }}>
        ...
        <main className="flex-1 flex flex-col h-full overflow-hidden relative w-full">
          ...
          <div 
            className="flex-1 overflow-hidden relative bg-surface-container-lowest" 
          ...
          >
            ...
            >
              <TransformComponent wrapperStyle={{ width: "100%", height: "100%" }}>
                <div id="bracket-container" className="w-[1400px] h-[900px] relative p-16 flex items-center justify-center">
                  ...
                  <div className="absolute inset-0 flex p-8" style={{ zIndex: 1 }}>
                    {rounds.map((roundNodes, rIndex) => (
                      <div key={\`round-\${rIndex}\`} className={\`flex flex-col justify-around w-[280px] \${rIndex > 0 ? 'ml-[80px]' : ''}\`}>
                        {roundNodes.map((node) => (
...
console.log("Updated BracketRoomClient.tsx with react-zoom-pan-pinch!");

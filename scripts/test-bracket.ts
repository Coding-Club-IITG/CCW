...
// ---------------------------------------------------------
// SSE BOT CLIENT (Per Room)
// ---------------------------------------------------------
async function runBotClient(botUser: any, roomId: string, teamId: string | null, mode: string, targetProblemIds?: string[]) {
  const headers = { 
  ...
  es.addEventListener("message", async (e: any) => {
    try {
      ...
      if (!isReady) {
        ...
        console.log(`[Bot ${botUser.name}] Sending READY signal for room ${roomId}...`);
  ...
  return new Promise((resolve) => {
    es.addEventListener("message", (e: any) => {
      try {
        ...
        if (payload && payload.type === "room.end") {
          console.log(`[Bot ${botUser.name}] Room ${roomId} match ended.`);
          es.close();
          resolve(true);
        }
      } catch (err) {}
    ...
    // Fail safe close if match takes too long
    setTimeout(() => {
      es.close();
      resolve(false);
    }, 15 * 60 * 1000); // 15 mins
...
async function orchestrateBots(botUsers: any[], contestId: string, mode: string, targetProblemIds?: string[]) {
    console.log(`\n▶️  Orchestrating ${botUsers.length} bots for bracket...`);
    const activeRoomsMap = new Set<string>();
    
    while(true) {
        const contest = await CustomContest.findById(contestId);
        if (!contest || contest.status === "completed") {
            console.log("\n🎉 Bracket Tournament Complete!");
            break;
        }

        const activeRooms = await ContestRoom.find({ 
            contestId, 
            status: "active" 
        }).lean();
        ...
        for (const room of activeRooms) {
            ...
            if (!activeRoomsMap.has(roomIdStr)) {
                activeRoomsMap.add(roomIdStr);
                console.log(`[Orchestrator] Found new active match room: ${room.name} (${roomIdStr})`);
                
                const teams = await ContestTeam.find({ roomId: room._id }).lean();
                
                for (const team of teams) {
                    for (const memberId of team.members) {
                        const bot = botUsers.find(b => b._id.toString() === memberId.toString());
                        if (bot) {
                            console.log(`[Orchestrator] Connecting Bot ${bot.name} to ${room.name}`);
                            runBotClient(bot, roomIdStr, team._id.toString(), mode, targetProblemIds).catch(() => {});
                        }
                    }
                }
            }
...
async function main() {
  try {
    console.log("=========================================");
    console.log("      CCW TEST BRACKET SIMULATOR         ");
    console.log("=========================================\n");

    const modeChoice = await askQuestion("Select Mode [1] Arena, [2] Blitz (default 2): ");
    const mode = modeChoice === "1" ? "arena" : "blitz";

    const formatChoice = await askQuestion("Select Format [1] 1v1, [2] Team-Tourney (default 1): ");
    const format = formatChoice === "2" ? "team-tournament" : "1v1";

    let maxContestants = 4;
    let teamSize = 1;

    const maxCStr = await askQuestion("Total Contestants/Teams in Bracket (e.g. 4 or 8): ");
    maxContestants = parseInt(maxCStr) || 4;
    ...
    const requiredBots = (maxContestants * teamSize) - 1; // 1 slot for dev user
    ...
    // For bracket testing, we supply dummy problemSlots so generateBracket doesn't fail
    // We assume max 3-4 rounds for 4-8 players
    const problemSlots = [];
    for(let i=0; i<4; i++) {
        problemSlots.push({
            roundIndex: i,
            problems: [{ problemId: `test_${i}_1` }, { problemId: `test_${i}_2` }, { problemId: `test_${i}_3` }]
        });
    }

    const contestDoc = await CustomContest.create({
      name: `Test Bracket ${mode.toUpperCase()} [${format}]`,
      description: "Automated test bracket contest",
      mode,
      format: "bracket", // Force bracket format
      visibility: "public",
      joinCode,
      startTime,
      endTime: new Date(startTime.getTime() + duration * 60000 * 5), // Long enough for all rounds
      ...
      minRating: 0,
      maxRating: 4000,
      maxParticipants: maxContestants * teamSize,
      teamSize,
    ...
    console.log(`Registration ends at: ${contestDoc.registrationSettings.deadline.toLocaleTimeString()}`);
    console.log(`Match (Round 1) starts at: ${startTime.toLocaleTimeString()}`);
    
    // Trigger Worker logic directly since Agenda / BullMQ will do it
    const { Queue } = require("bullmq");
    const queue = new Queue("reconciliation_queue", {
      connection: { url: process.env.REDIS_URL || "redis://localhost:6379" }
    });
    
    // For brackets, end_registration handles generation of brackets
    await queue.add("end_registration", { contestId: contestDoc._id.toString() }, { delay: regDelay * 1000 });
    // check_start kicks off round 1
    await queue.add("check_start", { contestId: contestDoc._id.toString() }, { delay: (regDelay + 60) * 1000 });
    console.log(`✅ Queued 'end_registration' and 'check_start' workers!`);
    ...
    // 4. Orchestrate bracket execution!
    await orchestrateBots(botUsers, contestDoc._id.toString(), mode, targetProblemIds);
    process.exit(0);

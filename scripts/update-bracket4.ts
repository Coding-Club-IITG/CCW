...
export default function BracketRoomClient({ contest, initialSnapshot }: { contest: ContestListingItem, initialSnapshot: BracketSnapshot }) {
  ...
  useEffect(() => {
    ...
    // Using a slightly longer timeout and tracking window resize to ensure layout settles
    const timeout = setTimeout(drawLines, 200);
  ...
  return (
    <>
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative dark stitch-container bg-background w-full text-on-surface font-body-md selection:bg-primary-container selection:text-on-primary-container" style={{ height: 'calc(100vh - 64px)' }}>
        <style>{\`
          .connector-line { stroke: #40493d; stroke-width: 2; fill: none; transition: stroke 0.3s; }
          .connector-line.active { stroke: #2e7d32; }
          .bracket-scroll::-webkit-scrollbar { height: 12px; width: 12px; }
          .bracket-scroll::-webkit-scrollbar-track { background: #1c1b1b; border-radius: 6px; border: 2px solid #111; }
          .bracket-scroll::-webkit-scrollbar-thumb { background: #40493d; border-radius: 6px; border: 2px solid #111; }
        ...
        <main className="flex-1 flex flex-col h-full overflow-hidden relative w-full">
          ...
          <div 
            className="flex-1 overflow-auto bracket-scroll relative bg-surface-container-lowest p-8 cursor-grab active:cursor-grabbing w-full h-full" 
            ref={scrollRef}
          ...
          >
            {/* The fixed inner canvas that ensures native overflow scrolling functions */}
            <div id="bracket-container" className="min-w-[1400px] min-h-[900px] relative p-8">
              <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ zIndex: 0 }}>
                {svgLines}
              </svg>

              <div className="absolute inset-0 flex p-8" style={{ zIndex: 1 }}>
                {rounds.map((roundNodes, rIndex) => (
                  <div key={\`round-\${rIndex}\`} className={\`flex flex-col justify-around w-[280px] \${rIndex > 0 ? 'ml-[100px]' : ''}\`}>
                    {roundNodes.map((node) => (
...
console.log("Updated BracketRoomClient.tsx back to native scrolling with height fix!");

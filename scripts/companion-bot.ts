...
async function run() {
  ...
  const botUser = users.find((u) => {
    ...
    return (
      !excludeNames.includes(un) &&
      !excludeNames.includes(nm) &&
      !excludeNames.includes(hd)
    );
  });
  ...
  if (!botUserId) {
    console.error(
      "Could not find a companion bot participant (non-dev user) in the room.",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (!readyRes.ok) {
      console.error(
        `[BOT] Ready endpoint failed with status: ${readyRes.status}`,
      );

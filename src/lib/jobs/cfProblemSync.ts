...
export async function syncCodeforcesProblems() {
  logger.info(
    "[CF-Problem-Sync] Starting Codeforces problem synchronization...",
  );
  await dbConnect();

  try {
    //  fetch all existing problemIds from database to perform incremental sync
    const existingQuestions = await ContestQuestion.find(
      {},
      { problemId: 1 },
    ).lean();
    const existingProblemIds = new Set(
      existingQuestions.map((q) => q.problemId),
    );
    const isFirstRun = existingProblemIds.size === 0;

    if (isFirstRun) {
      logger.info(
        "[CF-Problem-Sync] Database is empty. Performing full ingest...",
      );
    } else {
      logger.info(
        `[CF-Problem-Sync] Found ${existingProblemIds.size} existing problems. Running incremental sync...`,
      );
    }
    ...
    if (newProblems.length === 0) {
      logger.info(
        "[CF-Problem-Sync] No new problems found. Database is already up to date.",
      );
      return;
    }

    logger.info(
      `[CF-Problem-Sync] Found ${newProblems.length} new problems to sync.`,
    );

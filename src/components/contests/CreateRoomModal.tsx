...
import { createBracketContest, searchVerifiedUsers } from "@/lib/actions/admin/contests";
...
export default function CreateRoomModal({ isOpen, onClose, isAdmin = false, presets = [], deadlineMinutes = 1 }: { isOpen: boolean; onClose: () => void; isAdmin?: boolean; presets?: any[]; deadlineMinutes?: number }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        if (res.users) {
          const isUserInAnyTeam = (id: string) => manualTeams.some(t => t.members.some(m => m.id === id));
          const filtered = res.users.filter((u: any) => !registeredUsers.some(inv => inv.id === u.id) && !isUserInAnyTeam(u.id));
  ...
  const handleTopPresetChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (preset) {
      setFormData(prev => ({
        ...
        problemSelectionMode: preset.problemSelectionMode || prev.problemSelectionMode,
        ...
        fineTunedProblems: preset.problemSlots && preset.problemSlots.length > 0 ? preset.problemSlots.map((s: any) => s.problemId || "") : prev.fineTunedProblems,
        fineTunedProblemCount: preset.problemSlots && preset.problemSlots.length > 0 ? preset.problemSlots.length : prev.fineTunedProblemCount,
  ...
  useEffect(() => {
    ...
    setMaxPartError("");
  }, [formData.maxParticipants, formData.format, formData.teamSize, manualTeams.length, registeredUsers.length]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.format !== "bracket" && formData.problemSelectionMode === "fine-tuned") {
  ...
  const handleRegTimeAdd = (mins: number) => {
    ...
    date.setMinutes(date.getMinutes() + mins);
    date.setMinutes(date.getMinutes() - date.getTimezoneOffset());
    setFormData({ ...formData, registrationStartTime: date.toISOString().slice(0, 16) });
  };
  ...
  const renderProblemConfiguration = () => (
    <div className={`mt-[16px] pt-[16px] border-t border-outline-variant flex flex-col gap-[16px] ${!!topPresetId ? 'opacity-60 pointer-events-none' : ''}`}>
      <div className="flex items-center gap-2">
        ...
        {!!topPresetId && <span className="material-symbols-outlined text-[18px] text-on-surface-variant" style={{ fontVariationSettings: "'FILL' 1" }}>lock</span>}
      </div>
    ...
    <div className="fixed inset-0 flex items-center justify-center p-margin-mobile md:p-margin-desktop bg-black/60 backdrop-grayscale overflow-y-auto text-on-surface font-body-md" style={{ zIndex: 9999 }} onClick={onClose}>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-gutter custom-scrollbar">
          <form id="create-room-form" onSubmit={handleSubmit} className="flex flex-col gap-[16px]" spellCheck={false}>
            {isAdmin && (
              <div className="flex flex-col gap-unit bg-primary/5 p-4 rounded-lg border border-primary/20 mb-2">
                <label className="font-label-sm text-label-sm text-primary font-medium" htmlFor="top-preset-id">Load from Template (Optional)</label>
                <select
                  id="top-preset-id"
                  value={topPresetId}
                ...
                <span className="font-label-sm text-[11px] text-on-surface-variant mt-1">Selecting a template will auto-fill and lock the configuration below.</span>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-[16px]">
              ...
              <div className="flex flex-col gap-unit">
                ...
                >
                  ...
                  {isAdmin && <option value="bracket">Bracket (Knockout)</option>}
                </select>
              </div>
            </div>
            ...
            {formData.format === "bracket" && (
              <div className="mt-[16px] pt-[16px] border-t border-outline-variant flex flex-col gap-[16px]">
                <div className="flex items-center gap-2">
                  <h3 className="font-headline-lg-mobile text-headline-lg-mobile md:text-[24px] text-on-surface m-0">Bracket Settings</h3>
                  {!!topPresetId && <span className="material-symbols-outlined text-[18px] text-on-surface-variant" style={{ fontVariationSettings: "'FILL' 1" }}>lock</span>}
                </div>
                
                <div className={`flex flex-col gap-unit ${!!topPresetId ? 'opacity-60 pointer-events-none' : ''}`}>
                  <label className="font-label-sm text-label-sm text-on-surface-variant" htmlFor="preset-id">Match Preset</label>
                  <select
                    id="preset-id"
                  ...
                  <span className="font-label-sm text-label-sm text-primary mt-1">Bracket tournaments use presets to define the problem criteria for all rounds.</span>
                  {(() => {
                    const selectedMatchPreset = presets.find(p => p._id === formData.presetId);
                    return selectedMatchPreset ? (
                      <div className="mt-2 p-3 bg-surface-variant/30 rounded-lg border border-outline-variant/50 text-xs text-on-surface-variant flex flex-col gap-1">
                        <span className="font-semibold text-on-surface">{selectedMatchPreset.name}</span>
                        {selectedMatchPreset.description && <span>{selectedMatchPreset.description}</span>}
                        <div className="flex gap-3 mt-1 opacity-80">
                          <span className="capitalize">• {selectedMatchPreset.mode}</span>
                          {selectedMatchPreset.problemSelectionMode === 'bulk' ? (
                            <span>• {selectedMatchPreset.bulkProblemCount} problems ({selectedMatchPreset.bulkRatingMin}-{selectedMatchPreset.bulkRatingMax})</span>
                          ) : (
                            <span>• {selectedMatchPreset.fineTunedProblemCount} specific problems</span>
                          )}
                        </div>
                      </div>
                    ) : null;
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-[16px]">
                  <div className="flex flex-col gap-unit">
                    <label className="font-label-sm text-label-sm text-on-surface-variant" htmlFor="seeding-method">Seeding Method</label>
                    <select
                      id="seeding-method"
                      ...
                      onChange={e => {
                        ...
                        if (val === "manual") {
                          setFormData({ ...formData, seedingMethod: val, registrationType: "closed" });
                        } else {
                    ...
                    >
                      <option value="cf_rating">Codeforces Rating (Average)</option>
                  ...
                  <div className="flex flex-col gap-unit justify-center">
                    <label className="flex items-center gap-unit cursor-pointer mt-4">
                      <input
                        ...
                        onChange={e => setFormData({ ...formData, thirdPlacePlayoff: e.target.checked })}
                      ...
                      />
                      <span className="font-label-sm text-label-sm text-on-surface">Third Place Playoff</span>
                    </label>
                  </div>
                </div>
              </div>
            ...
            {/* New Scheduled Registration Section */}
                <div className="mt-4 p-4 border border-outline-variant rounded-lg bg-surface-container-low flex flex-col gap-4">
                  <h4 className="font-label-md text-on-surface mb-2 border-b border-outline-variant pb-2">Registration Window</h4>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-[16px]">
                    <div className="flex flex-col gap-unit">
                      <label className="font-label-sm text-label-sm text-on-surface-variant">Registration Type</label>
                      <select
                        value={formData.registrationType}
                        onChange={e => setFormData({ ...formData, registrationType: e.target.value })}
                        disabled={formData.format === "bracket" && formData.seedingMethod === "manual"}
                    ...
                    {formData.registrationType !== "closed" && (
                      <div className="flex flex-col gap-unit">
                        <label className="font-label-sm text-label-sm text-on-surface-variant">Registration Starts</label>
                        <select
                          value={formData.registrationStartMode}
                          onChange={e => setFormData({ ...formData, registrationStartMode: e.target.value })}
                          className="form-input rounded-lg w-full px-[12px] py-[8px] focus:outline-none transition-shadow appearance-none cursor-pointer"
                        >
                  ...
                  {formData.registrationType !== "closed" && formData.registrationStartMode === "schedule" && (
                    <div className="flex flex-col gap-unit mt-2">
                      <label className="font-label-sm text-label-sm text-on-surface-variant">Registration Start Time</label>
                      <div className="flex flex-col gap-[8px]">
                        ...
                        <div className="flex items-center gap-[8px] mt-2">
                          <button type="button" onClick={() => handleRegTimeAdd(3)} className="px-3 py-1 bg-surface-variant rounded text-[12px] hover:bg-primary/20">+3m</button>
                          <button type="button" onClick={() => handleRegTimeAdd(15)} className="px-3 py-1 bg-surface-variant rounded text-[12px] hover:bg-primary/20">+15m</button>
                          <button type="button" onClick={() => handleRegTimeAdd(60)} className="px-3 py-1 bg-surface-variant rounded text-[12px] hover:bg-primary/20">+1h</button>
                        </div>
                      </div>
                    </div>
                  )}
                    ...
                    <div className="flex flex-col gap-unit mt-2 border-t border-outline-variant pt-4">
                      ...
                      {useTeamsUI ? (
                        <div className="flex flex-col gap-4">
                            ...
                            >
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2 group border-b border-transparent hover:border-outline-variant focus-within:border-outline-variant transition-colors pb-0.5">
                                  ...
                                  />
                                  <span className="material-symbols-outlined text-[16px] text-on-surface-variant opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">edit</span>
                                </div>
                                <button
                                  type="button"
                                  onClick={() => setManualTeams(manualTeams.filter(t => t.id !== team.id))}
                              ...
                              <div className="flex flex-col gap-2 mt-1">
                                {team.members.map(member => (
                                  <div key={member.id} className="flex items-center justify-between p-2 bg-surface-variant rounded border border-outline-variant/50">
                                    <div className="flex items-center gap-2">
                                      {member.image ? (
                                      ...
                                      <span className="text-sm font-bold" style={{ color: getRatingColor(member.cfRating) }}>
                                    ...
                                    <button
                                      ...
                                      onClick={() => {
                                        const newTeams = [...manualTeams];
                                        newTeams[teamIndex].members = newTeams[teamIndex].members.filter(m => m.id !== member.id);
                                        setManualTeams(newTeams);
                                      }}
                                ...
                                {team.members.length < membersPerTeamLimit && (
                                  <div className="relative mt-1">
                                    <input
                                      ...
                                      value={activeSearchTeamId === team.id ? searchQuery : ""}
                                      onFocus={() => {
                                      ...
                                      onKeyDown={(e) => {
                                        if (e.key === 'ArrowDown') {
                                          ...
                                          e.preventDefault();
                                          if (searchResults.length > 0 && selectedUserIndex >= 0 && selectedUserIndex < searchResults.length) {
                                            const newTeams = [...manualTeams];
                                            newTeams[teamIndex].members.push(searchResults[selectedUserIndex]);
                                            setManualTeams(newTeams);
                                            setSearchQuery("");
                                            setSearchResults([]);
                                            setSelectedUserIndex(0);
                                    ...
                                    />
                                    {activeSearchTeamId === team.id && isSearching && (
                                      <div className="absolute right-3 top-2.5">
                                        <span className="material-symbols-outlined animate-spin text-on-surface-variant text-[18px]" style={{ fontVariationSettings: "'FILL' 0, 'wght' 400, 'GRAD' 0, 'opsz' 24" }}>refresh</span>
                                      </div>
                                    )}
                                    {activeSearchTeamId === team.id && searchQuery.length >= 2 && searchResults.length > 0 && (
                                          ...
                                          >
                                            ...
                                            <div className="flex flex-col">
                                              <div className="flex items-center">
                                                <span className="text-[14px] font-medium text-on-surface leading-tight">{user.name}</span>
                                                <span className="text-outline-variant mx-2 text-[12px]">|</span>
                                                <span className="text-[14px] font-bold leading-tight" style={{ color: getRatingColor(user.cfRating) }}>
                                                  {user.cfRating || 'Unrated'}
                                                </span>
                                              </div>
                                              <span className="text-[11px] text-on-surface-variant leading-tight mt-0.5">{user.cfHandle}</span>
                                            </div>
                                          </div>
                                        ))}
                                      </div>
                          ...
                          <button
                            type="button"
                            onClick={() => setManualTeams([...manualTeams, { id: crypto.randomUUID(), name: `Team ${manualTeams.length + 1}`, members: [] }])}
                            disabled={manualTeams.length >= Math.floor(formData.maxParticipants / membersPerTeamLimit)}
                          ...
                          >
                            ...
                            Add Team {manualTeams.length >= Math.floor(formData.maxParticipants / membersPerTeamLimit) && "(Max Limit Reached)"}
                          </button>
                        </div>
                      ) : (
                        <div className="flex flex-col gap-2">
                          {(formData.format !== "1v1" || registeredUsers.length < 2) && (
                            <div className="relative">
                              <input
                                type="text"
                                disabled={registeredUsers.length >= formData.maxParticipants}
                                placeholder={registeredUsers.length >= formData.maxParticipants ? "Max limit reached" : "Search to register participant..."}
                            ...
                            onKeyDown={(e) => {
                              if (e.key === 'ArrowDown') {
                                ...
                                e.preventDefault();
                                if (searchResults.length > 0 && selectedUserIndex >= 0 && selectedUserIndex < searchResults.length) {
                                  setRegisteredUsers(prev => [...prev, searchResults[selectedUserIndex]]);
                                  setSearchQuery("");
                                  setSearchResults([]);
                                  setSelectedUserIndex(0);
                                }
                          ...
                          {activeSearchTeamId === null && isSearching && (
                            <div className="absolute right-3 top-2.5">
                              <span className="material-symbols-outlined animate-spin text-on-surface-variant" style={{ fontVariationSettings: "'FILL' 0, 'wght' 400, 'GRAD' 0, 'opsz' 24" }}>refresh</span>
                            </div>
                          )}
                          {activeSearchTeamId === null && searchQuery.length >= 2 && searchResults.length > 0 && (
                            <div className="absolute z-50 w-full mt-1 bg-surface-container-high border border-outline-variant rounded-lg shadow-lg max-h-48 overflow-y-auto">
                              {searchResults.map((user, index) => (
                                <div 
                                  key={user.id}
                          ...
                          {formData.seedingMethod === "manual" ? (
                              ...
                              {registeredUsers.map((u, index) => (
                                <div 
                                  ...
                                  className={`flex items-center justify-between p-2 rounded-lg border transition-all duration-200 cursor-grab active:cursor-grabbing ${
                                    draggedUserIndex === index ? 'opacity-40 scale-[0.98]' : 
                                    dragOverIndex === index ? 'border-primary border-t-2 bg-primary/10 shadow-lg' :
                                    'bg-surface-variant border-outline-variant hover:border-primary/50'
                                  }`}
                                >
                                  <div className="flex items-center gap-3">
                                    <span className="material-symbols-outlined text-on-surface-variant/50 cursor-grab active:cursor-grabbing pointer-events-none">drag_indicator</span>
                                    <span className="font-bold text-on-surface-variant w-6 text-center text-[12px] pointer-events-none">#{index + 1}</span>
                                    {u.image ? (
                                      <img src={u.image} alt={u.name} className="w-8 h-8 rounded-full object-cover pointer-events-none" />
                                    ) : (
                                    ...
                                    <div className="flex flex-col pointer-events-none">
                                      <div className="flex items-center">
                                        <span className="text-[14px] font-medium text-on-surface">{u.name}</span>
                                        <span className="text-outline-variant mx-2 text-[12px]">|</span>
                                        <span className="text-[14px] font-bold" style={{ color: getRatingColor(u.cfRating) }}>
                                          {u.cfRating || 'Unrated'}
                                        </span>
                                      </div>
                                      <span className="text-[11px] text-on-surface-variant leading-tight mt-0.5">{u.cfHandle}</span>
                                    </div>
                                  </div>
                                  <div className="flex items-center gap-1">
                                    <button type="button" onClick={() => removeUser(u.id)} className="p-1 text-error hover:bg-error/10 rounded ml-2 flex items-center justify-center relative z-10"><span className="material-symbols-outlined text-[18px]">close</span></button>
                                  </div>
                                </div>
                              ))}
                            </div>
                            ...
                            <div className="flex flex-wrap gap-3 mt-4">
                              {registeredUsers.map(u => (
                                <div key={u.id} className="flex items-center bg-surface-variant border border-outline-variant rounded-full pl-2 pr-1 py-1.5 shadow-sm">
                                  {u.image ? (
                                  ...
                                  >
                                    <span className="material-symbols-outlined text-[16px]" style={{ fontVariationSettings: "'FILL' 0, 'wght' 400, 'GRAD' 0, 'opsz' 24" }}>close</span>
                                  </button>
                                </div>
                              ))}
                            </div>
                      ...
                      <span className="font-label-sm text-[11px] text-on-surface-variant mt-1">
                        These members will be automatically registered when the contest begins.
                      </span>
                    </div>
                  )}
              </div>

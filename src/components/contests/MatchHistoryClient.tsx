...
export default function MatchHistoryClient({
  ...
  return (
    <>
      <link
        href="https://fonts.googleapis.com/css2?family=Hanken+Grotesk:wght@100..900&family=Inter:wght@100..900&family=JetBrains+Mono:wght@100..900&display=swap"
        rel="stylesheet"
      />
      <link
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative dark stitch-container bg-background text-on-background font-body-md min-h-screen selection:bg-primary-container selection:text-on-primary-container">
        ...
        <main className="max-w-container-max-width mx-auto px-margin-mobile md:px-margin-desktop py-8 md:py-12 relative z-10 w-full flex-1 overflow-y-auto">
          ...
          <Link
            ...
            className="inline-flex items-center gap-2 text-on-surface-variant hover:text-primary transition-colors duration-200 font-label-sm text-label-sm mb-6"
          >
          ...
          <header className="mb-10">
            ...
            <p className="font-body-md text-body-md text-on-surface-variant">
              Review your recent algorithmic battles and performance metrics.
            </p>
          </header>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <div className="relative flex items-center">
                <select
                  ...
                  className="appearance-none bg-surface-container-highest border border-outline-variant text-on-surface font-label-sm text-label-sm rounded-lg py-2 pl-4 pr-10 hover:bg-surface-variant transition-colors focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary"
                >
          ...
          <div className="flex flex-col gap-4">
            {filteredHistory.map((contest) => {
              ...
              const borderClass = isVictory
                ...
                : isTie
                  ? "border-l-tertiary"
                  : "border-l-outline opacity-90";

              return (
                <div
                  key={contest._id}
                  className={`group bg-surface-container-low border-y border-r border-l-4 border-outline-variant ${borderClass} rounded-xl p-gutter flex flex-col md:flex-row items-start md:items-center justify-between gap-6 hover:bg-surface-container transition-colors duration-200`}
                >
                  {/* Left: Info */}
                  <div className="flex-1 flex flex-col gap-2">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      {isVictory ? (
                        ...
                        <span className="bg-tertiary-container text-on-tertiary-container font-label-sm text-label-sm px-2 py-0.5 rounded">
                          TIE
                        </span>
                      ) : (
                        <span className="bg-surface-variant text-on-surface font-label-sm text-label-sm px-2 py-0.5 rounded border border-outline">
                          DEFEAT
                        </span>
                      )}

                      <span className="bg-surface-variant text-on-surface-variant font-label-sm text-label-sm px-2 py-0.5 rounded border border-outline-variant capitalize">
                  ...
                  <div className="flex items-center justify-center min-w-[200px]">
                    <div className="text-center flex items-baseline gap-4">
                      <span
                        className={`font-headline-lg text-headline-lg ${isVictory ? "text-primary" : isTie ? "text-tertiary" : "text-on-surface-variant opacity-70"}`}
                      >
                        {outcomeScore}
                      </span>
                      {otherScores.map((score, idx) => (
                        <div key={idx} className="flex items-baseline gap-4">
                          ...
                          <span
                            className={`font-headline-lg text-headline-lg ${!isVictory && !isTie && score > outcomeScore ? "text-on-surface" : "text-on-surface-variant opacity-70"}`}
                          >
                            {score}
                          </span>
                        </div>
